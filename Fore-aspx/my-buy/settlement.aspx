<%@ Page Language="C#" AutoEventWireup="true" CodeFile="settlement.aspx.cs" Inherits="settlement" %>
    ...
    <form id="form1" runat="server">
        ...
        <table id="table_id" onclick="getSum()" border="2"; width="70%" bgcolor="#FEF2F1" cellpadding="2" style="margin:10px auto;">
            ...
            <asp:Repeater ID="Repeater2" runat="server" >
                <ItemTemplate>
                    <tr align="center">
                        <td style="height:80px;"><img src="../../Fore-images/<%# Eval("Gimage1")%>" alt="" style="height:70px; width: 70px;" /></td>
                        <input type="hidden" name="order_gno" value="<%# Eval("Gno")%>" />
                        <td><%# Eval("Gname")%></td>
                        <td><%# Eval("Gname")%></td>
                        <td><%# Eval("Gprice")%></td>
                        <input type="hidden" name="order_gprice" value="<%# Eval("Gprice")%>" />
                        <td><%# Eval("Number")%></td>
                        <input type="hidden" name="order_number" value="<%# Eval("Number")%>" />
                        <td id="item"><%# Eval("Totalmoney")%></td>
                        <input type="hidden" name="order_totalmoney" value="<%# Eval("Totalmoney")%>" />
                    </tr>
            ...
            <div id="light" class="white_content">
                <div style="width: 400px; margin: 20px auto; font-size: 16px; font-family:@Adobe 宋体 Std L;">
                    ...
                    <div style="width:200px; height:30px; margin:5px auto; /*border:1px solid #000;*/ text-align:center; padding-top:10px; border-radius:5px;">
                        <asp:Button ID="btn_order_evaluate" class="layui-btn confirm_btn" Text="确定" runat="server" style="height:35px; margin-left:20px; background-color:#EA5753;" OnClientClick = "document.getElementById('light').style.display='none';document.getElementById('fade').style.display='none'" OnClick="tijiao_btn_Click" CommandArgument=' <%# Eval("Gname")%>'/>
